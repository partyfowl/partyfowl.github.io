<template>
  <div>
    ...
    <figure class="album" v-for="image in images" :key="image.img">
        <div @click="loadImages('mock-api-response-2.json')" class="img-container"
             :style="{ backgroundImage: 'url(\'https://partyfowl.github.io/' + image.img + '\')' }"></div>
...
export default {
  ...
  mounted() {
      this.loadImages('mock-api-response.json');
  },
  methods: {
    test: function() {
      this.images = [];
    ...
    loadImages: function(uri) {
        axios.get(`https://partyfowl.github.io/${uri}`)
        .then(response => {
